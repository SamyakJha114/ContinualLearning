...
def load_and_preprocess_datasets(batch_size):
    ...
    dataset1 = load_dataset("krvhrv/Healix-2.8B-Token-Medical-Shot", split="train[0:10%]",cache_dir="./cache",num_proc = 32)
    dataset2 = load_dataset("Skylion007/openwebtext", split="train[0:10%]",cache_dir="./cache",num_proc = 32,trust_remote_code=True)
    ...
    train_dataloader_domain_3 = DataLoader(tokenized_datasets3, batch_size=batch_size, shuffle=True)
    ...
    test_dataloader_domain_3 = DataLoader(tokenized_datasets3, batch_size=batch_size, shuffle=True)
